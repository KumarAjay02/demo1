...
import { ITcsPayment, ITcsPaymentSearch, TcsPaymentSearch } from './model/tcs-payment.model';
import { TcsPaymentService } from './tcs-payment.service';
import { TcsPaymentDeleteDialogComponent } from './tcs-payment-delete-dialog.component';
...
@Component({
  ...
  templateUrl: './tcs-payment-list.component.html'
})
export class TcsPaymentListComponent implements OnInit {
  payments: ITcsPayment[] = [];
  filteredPayments: ITcsPayment[] = [];
  pagedPayments: ITcsPayment[] = [];
  searchParams: ITcsPaymentSearch = new TcsPaymentSearch();
  ...
  search(): void {
    this.paymentService.searchPayments(this.searchParams).subscribe(payments => {
      this.filteredPayments = payments;
      this.currentPage = 1;
      this.updatePagination();
    });
  ...
  delete(payment: ITcsPayment): void {
    const dialogRef = this.dialog.open(TcsPaymentDeleteDialogComponent, {
      data: { payment }
    });
  ...
  trackId(index: number, item: ITcsPayment): number {
    return item.id ?? 0;
  }
  ...
  updatePagination(): void {
    ...
    this.pagedPayments = this.filteredPayments.slice(startIndex, startIndex + this.itemsPerPage);
  }
