...
import { IRentPayment, IRentPaymentSearch, RentPaymentSearch } from './model/rent-payment.model';
import { RentPaymentService } from './rent-payment.service';
import { RentPaymentDeleteDialogComponent } from './rent-payment-delete-dialog.component';
...
export class RentPaymentListComponent implements OnInit {
  ...
  filteredPayments: IRentPayment[] = [];
  pagedPayments: IRentPayment[] = [];
  searchParams: IRentPaymentSearch = new RentPaymentSearch();
  ...
  delete(payment: IRentPayment): void {
    const dialogRef = this.dialog.open(RentPaymentDeleteDialogComponent, {
      data: { payment }
    });
  ...
  trackId(index: number, item: IRentPayment): number {
    return item.id ?? 0;
  }
