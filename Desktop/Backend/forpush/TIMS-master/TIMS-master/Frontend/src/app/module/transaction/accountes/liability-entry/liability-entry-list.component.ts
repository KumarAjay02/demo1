...
import { ILiabilityEntry, ILiabilityEntrySearch, LiabilityEntrySearch } from './model/liability-entry.model';
import { LiabilityEntryService } from './liability-entry.service';
import { LiabilityEntryDeleteDialogComponent } from './liability-entry-delete-dialog.component';
...
@Component({
  selector: 'app-liability-entry-list',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './liability-entry-list.component.html'
})
export class LiabilityEntryListComponent implements OnInit {
  entries: ILiabilityEntry[] = [];
  filteredEntries: ILiabilityEntry[] = [];
  pagedEntries: ILiabilityEntry[] = [];
  searchParams: ILiabilityEntrySearch = new LiabilityEntrySearch();
  ...
  delete(entry: ILiabilityEntry): void {
    const dialogRef = this.dialog.open(LiabilityEntryDeleteDialogComponent, {
      data: { entry }
    });
  ...
  trackId(index: number, item: ILiabilityEntry): number {
    return item.id ?? 0;
  }
