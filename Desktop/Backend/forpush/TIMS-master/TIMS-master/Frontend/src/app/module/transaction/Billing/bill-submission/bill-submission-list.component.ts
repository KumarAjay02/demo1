// bill-submission-list.component.ts
...
import { IBillSubmission, IBillSubmissionSearch, BillSubmissionSearch } from './model/bill-submission.model';
import { BillSubmissionService } from './bill-submission.service';
import { BillSubmissionDeleteDialogComponent } from './bill-submission-delete-dialog.component';
...
export class BillSubmissionListComponent implements OnInit {
  submissions: IBillSubmission[] = [];
  filteredSubmissions: IBillSubmission[] = [];
  pagedSubmissions: IBillSubmission[] = [];
  searchParams: IBillSubmissionSearch = new BillSubmissionSearch();
  ...
  loadAll(): void {
    this.submissionService.getSubmissions().subscribe(subs => {
  ...
  search(): void {
    this.submissionService.searchSubmissions(this.searchParams).subscribe(subs => {
      this.filteredSubmissions = subs;
      this.currentPage = 1;
      this.updatePagination();
    });
  ...
  delete(submission: IBillSubmission): void {
    const dialogRef = this.dialog.open(BillSubmissionDeleteDialogComponent, {
      data: { submission }
    });
  ...
  trackId(index: number, item: IBillSubmission): number {
    return item.id ?? 0;
  }
  ...
  updatePagination(): void {
    ...
    this.pagedSubmissions = this.filteredSubmissions.slice(startIndex, startIndex + this.itemsPerPage);
  }
